...
export default {
  ...
  lookup: {
    title: 'Проверить Matrix-сервер',
    description: 'Нотариус возвращает verify keys из кэша или только что полученные с origin.',
    field: 'Matrix-сервер',
    placeholder: 'matrix.org',
    hint: 'Только hostname. MXKeys сам выполняет Matrix discovery (well-known, SRV, порты).',
    submit: 'Проверить ключи',
    submitting: 'Проверка…',
    validation: {
      empty: 'Введите имя Matrix-сервера, например matrix.org',
      tooLong: 'Слишком длинное имя хоста',
      badShape: 'Введите hostname вроде matrix.org. Порт не нужен, если нестандартный.',
      badPort: 'Порт должен быть от 1 до 65535',
    },
    notfound: {
      title: 'MXKeys не нашёл ключи для {{server}}.',
      withPort: 'Сервер недоступен на этом порту или это не Matrix-сервер. Попробуйте другой порт или hostname.',
      withoutPort: 'Большинство серверов находятся автоматически. Если federation на нестандартном порту, попробуйте:',
      tryPort: 'Попробовать',
    },
    failed: 'Не удалось выполнить запрос.',
    result: {
      validUntil: 'действителен до',
      footer: 'Результат для {{server}}.',
    },
    about: {
      title: 'О сервере {{server}}',
      rows: {
